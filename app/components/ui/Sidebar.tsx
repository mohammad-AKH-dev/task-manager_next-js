  ...
  return (
    <>
      <motion.div
        ...
        animate={{
          width: animate ? (open ? "100%" : "60px") : "100%",
        }}
...
export const MobileSidebar = ({
  ...
  return (
    <>
      <div
        className={cn(
          "h-10 px-4 py-4 flex flex-row relative z-[999] md:hidden pr-[1.5rem] items-center justify-between bg-neutral-100 dark:bg-neutral-800 w-full"
        )}
        {...props}
      >
        <div className="flex open-btn justify-start -translate-x-2 min-w-[24px] min-h-[24px] z-20 w-full">
          <IconMenu2
        ...
        <AnimatePresence>
          {open && (
            <motion.div
              ...
              className={cn(
                "fixed h-full w-full inset-0 mt-16 bg-white dark:bg-neutral-900 p-10 z-[9999] flex flex-col justify-between",
                className
              )}
            >
              <div
                className="absolute right-10 top-16 z-50 text-neutral-800 dark:text-neutral-200"
                onClick={() => setOpen(!open)}
              >
                <IconX />
              </div>
